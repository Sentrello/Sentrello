...
/**
 * An earlier decision: `defaults.ts` grants `settings:["read"]` to executives, managers
...
test("resetting a password writes the address, so it can clear an account lock", async () => {
  // An earlier change's addendum: this is the remedy an administrator actually reaches
...
test("the recent-changes history excludes sign-in noise", async () => {
  // Carried from An earlier change's review: once sign-in events exist, twenty-five bot
...
/**
 * An earlier change's Definition of Done walk found this: the lock had been derivable
 * since An earlier change and clearable by route since an earlier change, and no screen knew there
