...
import { and, db, eq, gte, inArray, lte, schema, sql } from "@sentrello/db";
...
/**
 * What this holds about a person, registered where the table lives.
 *
 ...
 * deleted them because somebody asked would be unable to answer the IRS.
 ...
 * to an erasure request is "no, and here is why" — which is what this
 * registers, rather than an `erase`.
 *
 * This used to be registered from the Free half's own `personal-data.ts`,
 * which meant the Free package named a table it does not own. Registered
 * here instead, beside `contractorTaxDetails` itself, so that when this file
 * moves into the paid bundle the registration goes with it rather than being
 * left behind as a reference to a table the Free package can no longer see.
 */
function registerContractorPersonalData(ctx: ModuleContext) {
  ctx.registerPersonalData({
    ...
    // No `erase`. See the note above: the obligation is the business's own
...
export function registerContractors(
  ...
  registerContractorPersonalData(ctx);
